...
ComNoRSD=np.zeros(len(BETAS))
SigmaNoRSD=np.zeros(len(BETAS))
...
for i in range(len(BETAS)):
    ...
    ComNoRSD[i]=np.mean(C_temporal)
    SigmaNoRSD[i]=np.std(C_temporal)
    
ComRSD=np.zeros(len(BETAS))
SigmaRSD=np.zeros(len(BETAS))

for i in range(len(BETAS)):
    C_temporal=np.zeros(20)
    for j in range(20):
        #se lee los datos random
        A=np.loadtxt('rsd_random_sphere'+numeros[j]+'_'+str(BETAS[i])+'.BSKIndex', usecols=0)
        B=np.loadtxt('rsd_random_sphere'+numeros[j]+'_'+str(BETAS[i])+'.BSKIndex', usecols=1)
        C=np.concatenate((A,B),axis=0)
    
        nodos_ran, conexiones_ran = np.unique(C, return_counts=True)
    
        #se lee los datos simulados
        A=np.loadtxt('rsd_sphere'+numeros[j]+'_'+str(BETAS[i])+'.BSKIndex', usecols=0)
        B=np.loadtxt('rsd_sphere'+numeros[j]+'_'+str(BETAS[i])+'.BSKIndex', usecols=1)
        C=np.concatenate((A,B),axis=0)
        ...
        #se analiza random
        puntos_ran=np.loadtxt('rsd_random_sphere'+numeros[j]+'.dat',usecols=0)
        total_ran=len(puntos_ran)
        nonzero_ran=len(nodos_ran)
        p0_ran=np.array([(total_ran-nonzero_ran)])
        P=np.concatenate((p0_ran,y_ran),axis=0)
        y_ran=P/total_ran
        #se analiza datos
        puntos_dat=np.loadtxt('rsd_sphere'+numeros[j]+'.dat',usecols=0)
        total_dat=len(puntos_dat)
        nonzero_dat=len(nodos_dat)
        p0_dat=np.array([(total_dat-nonzero_dat)])
        P=np.concatenate((p0_dat,y_dat),axis=0)
    ...
    ComRSD[i]=np.mean(C_temporal)
    SigmaRSD[i]=np.std(C_temporal)


plt.figure(figsize=(5,4))
plt.errorbar(BETAS, ComNoRSD, SigmaNoRSD,label='No RSD')
plt.errorbar(BETAS, ComRSD, SigmaRSD,label='RSD')
plt.xlabel('\u03b2')
plt.ylabel('C')
plt.legend()
plt.savefig('CvBeta_RSD.png')
